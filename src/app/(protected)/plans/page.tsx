...
const plans: PlanCard[] = [
  {
    ...
    features: ["1 veículo", "Registro manual de manutenções", "Histórico básico", "Lembretes simples"],
    missing: ["Exportar relatório PDF", "Selo de verificação", "Score de confiabilidade", "Sem anúncios"],
    highlight: false,
  },
  {
    id: "pro",
...
export default function PlansPage() {
  ...
  function handleUpgrade(plan: PlanCard) {
    if (plan.id === currentPlan) return;
    toast.info("Pagamentos estão em desenvolvimento. Em breve você poderá fazer upgrade aqui.");
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="px-5 py-6 pb-10 max-w-lg mx-auto space-y-5">
        ...
        {plans.map((plan, i) => {
          ...
          return (
            <div
              ...
              } ${isCurrent && !plan.highlight ? "ring-2 ring-primary" : ""}`}
            ...
            >
              ...
              {isCurrent && (
                <span className={`absolute -top-3 right-4 text-[10px] font-bold px-3 py-1 rounded-full ${
                  plan.highlight ? "bg-primary-foreground text-primary" : "bg-primary text-primary-foreground"
                }`}>
                  Plano atual
                </span>
              )}
              ...
              <div className="space-y-2.5 mb-4">
                {plan.features.map((f) => (
                  <div key={f} className="flex items-center gap-2.5">
                    <Check className={`w-4 h-4 flex-shrink-0 ${plan.highlight ? "text-accent-foreground" : "text-accent"}`} />
        ...
        <p className="text-center text-xs text-muted-foreground pt-2">
          Pagamentos em desenvolvimento · Cancele a qualquer momento
        </p>
      </div>
    </div>
  );
